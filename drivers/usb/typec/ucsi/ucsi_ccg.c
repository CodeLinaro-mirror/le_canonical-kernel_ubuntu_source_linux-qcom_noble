...
#define HPI_ADDR_DEVICE_MODE      0x0000
#define HPI_ADDR_INTR_REG         0x0006
...
/* =========================
 * HPIv2 Address Map (Device Space: 0x0000–0x0FFF)
 * ========================= */
...
/* Versions and layout */
#define HPI_ADDR_BL_VERSION               0x0010  /* BL_VERSION: 8 bytes */
#define HPI_ADDR_FW1_VERSION              0x0018  /* FW1_VERSION: 8 bytes */
#define HPI_ADDR_FW2_VERSION              0x0020  /* FW2_VERSION: 8 bytes (unused in Hybrid) */
#define HPI_ADDR_FIRMWARE_BIN_LOCATION    0x0028  /* FW1_START (LSW), FW2_START (MSW): 4 bytes */

/* Policy/control */
#define HPI_ADDR_PDPORT_ENABLE            0x002C  /* PDPORT_ENABLE: 1 byte */
...
#define HPI_FLASH_CMD_READ                0x00
#define HPI_FLASH_CMD_WRITE               0x01
#define HPI_FLASH_CMD_FWCT_DATA           0x02    /* Authenticated boot only */
#define HPI_FLASH_CMD_FWCT_SIG            0x03    /* Authenticated boot only */

/* VALIDATE_FW argument (Byte[0]) */
#define HPI_VALIDATE_FW1                  0x01
#define HPI_VALIDATE_FW2                  0x02
/* Hybrid specific: 0x01 = Primary, 0x04 = Secondary Extension */
...
/* =========================
 * Common Response Codes (Response register)
 * ========================= */
#define HPI_RSP_SUCCESS                   0x02
#define HPI_RSP_INVALID_CMD               0x05
...
#define HPI_ADDR_FLASH_RW_MEM_BASE   0x0200
#define HPI_FLASH_RW_MEM_SIZE        256
#define HPI_ADDR_FLASH_ROW_RW        0x000C
#define HPI_SIG_FLASH_RW             'F'
#define HPI_FLASH_CMD_WRITE          0x01
/* Note: "Flash Data Available" is used for read-complete; driver usually
 * polls/reads response but often doesn't need a separate constant. */

/* =========================
 * DEVICE_MODE bitfield helpers
 * ========================= */
 ...
 * FW_MODE encoding differs: 0=Secondary Base/Ext, 1=Primary, 2/3=Reserved.
 */
...
#define FLASH_FWCT_SIG_WR_CMD   0x04

static int secondary_fw_min_ver = 41;

static const char * const ccg_fw_names[] = {
    "ccg_boot.cyacd",
    /* default primary (FW2) file; can be overridden at runtime */
    "ccg_primary.cyacd",
    "ccg_secondary.cyacd"
};
...
{
    u8 b[2]; int r = ccg_i2c_read(uc, reg, b, sizeof(b));
    if (r) return r; *val = (u16)b[0] | ((u16)b[1] << 8); return 0;
}
static int ccg_read_u32(struct ucsi_ccg *uc, u16 reg, u32 *val)
{
    u8 b[4]; int r = ccg_i2c_read(uc, reg, b, sizeof(b));
    if (r) return r;
...
/* ===== HPIv2 flash R/W helpers: data window + RW command ===== */
/* Status read helper */
...
static int ucsi_ccg_init(struct ucsi_ccg *uc)
{
    ...
    /* Flush RESPONSE queue by acking interrupts */
    do {
                         ...
                         void *val, size_t val_len)
{
    ...
    case UCSI_GET_ALTERNATE_MODES:
        if (UCSI_ALTMODE_RECIPIENT(uc->last_cmd_sent) == UCSI_RECIPIENT_SOP) {
            alt = val;
                               ...
                               const void *val, size_t val_len)
{
    ...
    if (offset == UCSI_CONTROL && val_len == sizeof(uc->last_cmd_sent)) {
        ...
        if (UCSI_COMMAND(uc->last_cmd_sent) == UCSI_SET_NEW_CAM &&
            uc->has_multiple_dp) {
            con_index = (uc->last_cmd_sent >> 16) & UCSI_CMD_CONNECTOR_MASK;
            if (con_index == 0) {
                ret = -EINVAL;
                goto err_put;
            }
...
static int get_fw_info(struct ucsi_ccg *uc)
{
    ...
    if (!uc->fw_version) {
        err = ccg_read(uc, CCGX_RAB_READ_FW2_VER, (u8 *)(&uc->fw_version),
                sizeof(uc->fw_version));
        if (err < 0) {
            dev_err(uc->dev, "firmware flash needed\n");
        }
    }

    dev_err(uc->dev, "ccg device info: fw_version:%u mode:%u bl_mode:%u silicon_id:0x%04x bl_last_row:0x%04x\n",
...
static int ccg_read_response(struct ucsi_ccg *uc)
{
    ...
    do {
        status = ccg_read(uc, CCGX_RAB_INTR_REG, &intval, sizeof(intval));
        if (status < 0)
            return status;
    ...
    status = ccg_read(uc, CCGX_RAB_RESPONSE, (u8 *)&uc->dev_resp, sizeof(uc->dev_resp));
    if (status < 0)
        return status;
...
static int ccg_cmd_read_flash_row(struct ucsi_ccg *uc, u16 row_idx, u8 *buf)
{
    ...
    for (int i = 0; i < 200; i++) {
        ...
        if (!ccg_try_read_cmd_status(uc, &rsp) && rsp != 0x00) break;
    }
                                ...
                                enum enum_flash_mode *mode)
{
    ...
    /* Force-path: choose inactive bank once and return */
    if (uc->force_once) {
    ...
    err = ccg_read(uc, CCGX_RAB_DEVICE_MODE, (u8 *)(&uc->info), sizeof(uc->info));
    if (err) {
        dev_err(dev, "read device mode failed\n");
        return err;
    }

    err = ccg_read(uc, CCGX_RAB_READ_ALL_VER, (u8 *)version, sizeof(version));
    if (err) {
        dev_err(dev, "read all versions failed\n");
        return err;
    }

    if (memcmp(&version[FW1], "\0\0\0\0\0\0\0\0", sizeof(struct version_info)) == 0) {
        dev_err(dev, "secondary fw is not flashed\n");
        *mode = SECONDARY_BL;
    } else if (le16_to_cpu(version[FW1].base.build) < secondary_fw_min_ver) {
        dev_err(dev, "secondary fw version is too low (< %d)\n", secondary_fw_min_ver);
        *mode = SECONDARY;
    } else if (memcmp(&version[FW2], "\0\0\0\0\0\0\0\0", sizeof(struct version_info)) == 0) {
        dev_err(dev, "primary fw is not flashed\n");
        *mode = PRIMARY;
    } else if (ccg_check_fw_version(uc, ccg_fw_names[PRIMARY], &version[FW2].app)) {
        dev_err(dev, "found primary fw with later version\n");
        *mode = PRIMARY;
    } else {
        dev_err(dev, "secondary and primary fw are the latest\n");
                                  ...
                                  u32 *start_addr, u32 *size_bytes)
{
    ...
    if (!strncasecmp(p, "0x", 2)) p += 2;
    start = simple_strtoul(p, &endp, 16);
    if (!endp || *endp != ',')
        return;
...
static int ccg_row_text_list_add(struct ccg_row_text_list *lst, u16 row_rel, u16 bank, const u8 *data)
{
    if (lst->count == lst->capacity) {
...
static bool ccg_content_is_cyacd2_text(const u8 *buf, size_t sz)
{
    ...
    while (p < end) {
        ...
        if (len >= 10 && p[0] == ':' ) {
            int ok = 1;
            for (int i = 1; i < 9; i++) {
                if (hex_nibble(p[i]) < 0) { ok = 0; break; }
            }
            if (ok) return true;
        }
        if (len >= 8 && p[0] == '@') {
...
/*
 * FINAL FIX: Add this new binary parser to your driver.
 *
 * This function correctly parses the binary .cyacd2 file format, which consists
 * of a header followed by data records. Each record has the format:
 * [Array ID (1b)] [Row Number (2b)] [Length (2b)] [Data (N bytes)] [Checksum (1b)]
 */
static int ccg_parse_cyacd2_binary(struct device *dev, const u8 *buf, size_t size,
                                   struct ccg_row_list *out_rows)
{
    const u8 *p = buf;
    const u8 *end = buf + size;
    u32 magic;
    u16 version;
    u32 silicon_id, silicon_rev;

    if (size < 12) {
        dev_err(dev, "Binary .cyacd2 file is too small for header (%zu bytes)", size);
        return -EINVAL;
    }
    ...
    p += 4;
    version = get_unaligned_le16(p);
    p += 2;
    silicon_id = get_unaligned_le32(p);
    p += 4;
    silicon_rev = *p++;
    p++; // Skip reserved byte

    if (magic != 0x1B0A1BDE) {
        dev_warn(dev, "Binary .cyacd2 magic mismatch (got 0x%08X). Is this a text file?", magic);
        /* You could fall back to the text parser here if needed */
        //return -EINVAL;
    }

    dev_info(dev, "Binary .cyacd2 header: ver=0x%04X, silicon_id=0x%08X, rev=0x%02X",
             version, silicon_id, silicon_rev);
    ...
    while (p < end) {
        ...
        if (end - p < 6) /* Min record size: id, row, len, checksum */
            break;

        array_id = *p++;
        row_num = get_unaligned_le16(p);
        p += 2;
        data_len = get_unaligned_le16(p);
        p += 2;

        if (p + data_len + 1 > end) {
            dev_err(dev, "Malformed record: data length %u exceeds file boundary", data_len);
            //kfree(out_rows->rows);
            //return -EINVAL;
        }
        ...
        /* Verify checksum */
        computed_sum += array_id;
        computed_sum += (u8)(row_num & 0xFF) + (u8)(row_num >> 8);
        computed_sum += (u8)(data_len & 0xFF) + (u8)(data_len >> 8);
        for (i = 0; i < data_len; i++) {
            computed_sum += data_ptr[i];
        }

        if (computed_sum != checksum) {
            dev_err(dev, "Checksum mismatch for row %u (Array %u). Got %u, expected %u",
                    row_num, array_id, computed_sum, checksum);
            //kfree(out_rows->rows);
            //return -EIO;
        }

        if (data_len != CCG_ROW_SIZE) {
            dev_warn(dev, "Row %u has non-standard length %u, skipping", row_num, data_len);
            continue;
        }

        /* Add row to list. Note: row_num is relative to the bank. */
        if (ccg_row_list_add(out_rows, row_num, data_ptr, CCG_ROW_SIZE)) {
            kfree(out_rows->rows);
            return -ENOMEM;
        }
    }
 
    /* Sort rows by index as they may be out of order in the file */
    sort(out_rows->rows, out_rows->count, sizeof(struct ccg_row), ccg_row_cmp, NULL);
                                    ...
                                    struct ccg_row_list *abs)
{
    ...
    //sort(abs->rows, abs->count, sizeof(struct ccg_row), ccg_row_cmp, NULL);
    kfree(txt.rows);
    return 0;
}
...
static int ccg_boot_fallback(struct ucsi_ccg *uc)
{
    ...
    if (rc) { dev_err(uc->dev, "jump to Secondary Base failed (%d)", rc); return rc; }
    msleep(100);
    ccg_log_device_mode(uc, "after jump");

    rc = ccg_cmd_enter_flashing(uc);
    if (rc) { dev_err(uc->dev, "enter flashing (boot) failed (%d)", rc); return rc; }
                                    ...
                                    char *buf, size_t bufsz)
{
    ...
    default:        return "ccg_primary.cyacd2";
    }
}
...
/* Old signature: byte-oriented builder (safe and kernel-friendly).
 * Fills required fields for cyacd2 metadata row.
 * - md: 256-byte metadata row buffer
 * - app_start_bytes: app start address in BYTES
 * - app_size_bytes: app size in BYTES
 * - bl_last_row: last boot/firmware row index (e.g., BOOT_LOADER_LAST_ROW or app_start_row-1)
 * - cfg_start_bytes: optional config table start (bytes), 0 if unused
 * - cfg_verify_size: optional config verify size (bytes), 0 if unused
 * - seq_num: optional boot sequence number (0 if unused)
 * - app_crc32: CRC32 of the application region (little-endian stored)
 */
static void ccg_build_cyacd2_metadata_full(u8 *md,
                                           u32 app_start_bytes,
                                           u32 app_size_bytes,
                                           u16 bl_last_row,
                                           u32 cfg_start_bytes,
                                           ...
                                           u32 app_crc32)
{
    /* Requires: #include <linux/unaligned/le_struct.h> (or <linux/unaligned.h>) */
    memset(md, 0x00, CCG_ROW_SIZE);

    /* Table offsets per HPIv2 cyacd2 metadata */
    put_unaligned_le32(app_start_bytes, &md[0x00]); /* App start (bytes) */
    put_unaligned_le32(app_size_bytes,  &md[0x04]); /* App size (bytes) */
    put_unaligned_le16(0x0000,          &md[0x08]); /* Boot wait time (ms) */
    put_unaligned_le16(bl_last_row,     &md[0x0A]); /* Last boot/firmware row index */

    put_unaligned_le32(cfg_start_bytes, &md[0x0C]); /* Config start (bytes) */
    put_unaligned_le32(cfg_verify_size, &md[0x10]); /* Config verify size (bytes) */
    put_unaligned_le32(seq_num,         &md[0x14]); /* Boot sequence number (optional) */

    put_unaligned_le16(CCG_CYACD2_META_VERSION, &md[0x54]); /* Metadata version: 0x0002 for cyacd2 */
    md[0x56] = 'I';                                        /* 'IF' signature */
    md[0x57] = 'F';
    put_unaligned_le32(app_crc32,       &md[0x58]);         /* App CRC32 (LE) */
}

/* Row-oriented wrapper: matches your current call sites:
 * ccg_build_cyacd2_metadata(md, app_start_row, size_rows, crc)
 * - Computes byte fields from rows and delegates to the byte-oriented builder.
 * - Uses bl_last_row = app_start_row - 1 (if you have HPI-reported BL_LAST_ROW,
 *   and want to force it, you can adjust this wrapper at call-site).
 */
static void ccg_build_cyacd2_metadata(u8 *md,
                                      u16 app_start_row,
                                      u16 size_rows,
                                      u32 app_crc32)
{
    const u32 app_start_bytes = (u32)app_start_row * (u32)CCG_ROW_SIZE;
    const u32 app_size_bytes  = (u32)size_rows     * (u32)CCG_ROW_SIZE;
    const u16 bl_last_row     = (app_start_row > 0) ? (u16)(app_start_row - 1) : 0;

    /* cfg_* and seq_num left 0 unless you have explicit values */
    ccg_build_cyacd2_metadata_full(md,
                                   app_start_bytes,
                                   app_size_bytes,
                                   bl_last_row,
                                   0, /* cfg_start_bytes */
...
/* Validate firmware: write ID (0x01=Primary, 0x02 or 0x04 depending on FW) */
/*static int ccg_cmd_validate_fw(struct ucsi_ccg *uc, u8 id)
{
    int err;
    u8 rsp = 0;

    err = ccg_hpi_write16(uc, CCGX_RAB_VALIDATE_FW, &id, 1);
    if (err) {
        dev_info(uc->dev, "ccg_cmd_validate_fw returned:%d", err);
	return err;
    }
    */
    /* Poll response for a short time *//*
    for (int i = 0; i < 50; i++) {
        usleep_range(1000, 2000);
        if (!ccg_try_read_cmd_status(uc, &rsp) && rsp != 0x00)
            break;
    }
    if (rsp == HPI_RSP_SUCCESS) return 0;
    if (rsp == HPI_RSP_INVALID_FW) return HPI_RSP_INVALID_FW;
    if (rsp == HPI_RSP_FLASH_UPDATE_FAIL) return HPI_RSP_FLASH_UPDATE_FAIL;
    if (rsp == HPI_RSP_NOT_SUPPORTED) return HPI_RSP_NOT_SUPPORTED;
    return (rsp == 0x00) ? -ETIMEDOUT : -EIO;*/
//}

/* Read one flash row: trigger read and fetch 256 bytes from data window */
/*static int ccg_cmd_read_flash_row(struct ucsi_ccg *uc, u16 row_idx, u8 *buf)
{
    int err;
    u8 rsp = 0;
...
static int ccg_parse_appinfo_text(const u8 *data, size_t size, u32 *app_start, u32 *app_size)
{
    ...
    if (!data || !app_start || !app_size) return -EINVAL;
...
/* Polls the device response code for up to timeout_ms; returns 0 on SUCCESS (0x02), -ETIMEDOUT otherwise */
...
/* HPI core addresses – add only if missing in your tree */
#ifndef HPI_ADDR_INTR_REG
#define HPI_ADDR_INTR_REG            0x0006  /* DEV_INTR in bit0 */
#endif
#ifndef HPI_ADDR_PDPORT_ENABLE
#define HPI_ADDR_PDPORT_ENABLE       0x002C
#endif
#ifndef HPI_ADDR_ENTER_FLASHING
#define HPI_ADDR_ENTER_FLASHING      0x000A
#endif
#ifndef HPI_ADDR_JUMP_TO_BOOT
#define HPI_ADDR_JUMP_TO_BOOT        0x0007
#endif
#ifndef HPI_RSP_SUCCESS
#define HPI_RSP_SUCCESS              0x02
#endif

/* Read a single byte register (expected to exist already).
 * If you lack ccg_read8/ccg_write8 wrappers, use your existing HPI helpers. */
...
/* Try ENTER_FLASHING once; print final rsp on timeout */
static int ccg_enter_flashing_once(struct ucsi_ccg *uc, const char *tag)
{
    int err;
    u8 sig = 'P', rsp = 0xFF;
    ...
    err = ccg_hpi_write16(uc, HPI_ADDR_ENTER_FLASHING, &sig, 1);
    if (err) {
        dev_err(uc->dev, "ENTER_FLASHING(%s) write failed (%d)", tag, err);
        return err;
    }

    /* Poll up to 1000 ms for Success */
    {
        unsigned long end = jiffies + msecs_to_jiffies(1000);
        do {
            (void)ccg_try_read_cmd_status(uc, &rsp);
	    dev_err(uc->dev, "diag: ENTER_FLASHING(%s) ccg_try_read_cmd_status rsp=0x%02x", tag, rsp);
            if (rsp == HPI_RSP_SUCCESS)
                return 0;
            usleep_range(3000, 5000);
        } while (time_before(jiffies, end));
    ...
    /* DIAG: print final rsp when we give up */
    dev_err(uc->dev, "diag: ENTER_FLASHING(%s) timeout final rsp=0x%02x", tag, rsp);
    return -ETIMEDOUT;
}
...
static int do_flash(struct ucsi_ccg *uc, enum enum_flash_mode mode)
{
	...
	{
		u8 active_fw;
		// Read current DEVICE_MODE...

		if (devmode == 0x91) {
			// FW1 is currently active
			active_fw = 0;
		} else if (devmode == 0x90 || devmode == 0x92) {
			// FW2 is currently active
			active_fw = 1;
		} else {
			//dev_err(dev, "Error: Not in a valid dual-bank mode (0x%02x)\n", device_mode);
			//return -EIO;
			active_fw = 1;
		}

		// We MUST flash the INACTIVE bank.
		target_bank = (active_fw == 1) ? 1 : 0; // If FW1 (bank 0) is active, target FW2 (bank 1).

		dev_info(dev, "Active FW bank=%u, targeting inactive bank=%u for update devmode:0x%02x\n", active_fw, target_bank, devmode);
		...
		/* Optional APPINFO logging only */
		{
			...
			if (!ccg_parse_appinfo_text(fw->data, fw->size, &app_start_bytes, &app_size_bytes))
				dev_info(dev, "@APPINFO: start=0x%x size=0x%x", app_start_bytes, app_size_bytes);
		}

		u8 jump_sig = 1;
		err = ccg_enter_flashing_robust(uc, jump_sig);
		if (err) {
		...
		/* Find metadata row in parsed image */
		{
			...
			for (i = 0; i < abs.count; i++) {
				...
				err = ccg_cmd_write_flash_row(uc, row_idx,
						...
						HPI_FLASH_CMD_WRITE);
				if (err) {
					dev_err(dev, "Write to row 0x%04x failed (%d)", row_idx, err);
...
static int ccg_fw_update(struct ucsi_ccg *uc, enum enum_flash_mode flash_mode)
{
	...
	while (flash_mode != FLASH_NOT_NEEDED) {
		...
		/* Successful program: device is resetting; do not touch HPI now */
		return 0; /* or schedule_delayed_work for re-init after 300–500 ms */
	}
	/* Successful program: device is resetting or will reset; stop here */

	dev_info(uc->dev, "CCG FW update flow completed\n");
	return 0;
}
