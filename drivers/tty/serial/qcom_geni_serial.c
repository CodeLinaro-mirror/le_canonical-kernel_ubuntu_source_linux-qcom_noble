...
static void qcom_geni_serial_handle_tx_dma(struct uart_port *uport)
{
	struct qcom_geni_serial_port *port = to_dev_port(uport);
	struct circ_buf *xmit = &uport->state->xmit;
	unsigned int circ_pending = uart_circ_chars_pending(xmit);

	/*
	 * Only advance the circ_buf if it still contains the bytes that were
	 * transferred. uart_flush_buffer() may have run before this IRQ
	 * fired: it calls uart_circ_clear() under the port lock, making
	 * uart_circ_chars_pending() = 0 while tx_remaining remains non-zero.
	 * Calling uart_xmit_advance() in that case would advance xmit->tail
	 * past xmit->head (which was zeroed by the clear), causing
	 * uart_circ_chars_pending() to return UART_XMIT_SIZE - tx_remaining
	 * and triggering a spurious large DMA transfer of stale data.
	 */
	if (circ_pending >= port->tx_remaining)
